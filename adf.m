...
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
% Descripción: Carry out DF tests on a time-series vector
% Inputs:
...
%         >1, for higher order polynomial
%   l   : Max lag for automatic search
%   ic  : Information criteria: 'AIC','HQC','BIC'
%
% Outputs:
%   results: 
%   -.meth  : 'adf'
...
%   -.adf   : ADF t-statistic
%   -.crit  : Col vector of critical values [1% 5% 10% 90% 95% 99%].
...
results.nlag  = lopt;
results.alpha = res_ols.b(end);
results.adf   = (res_ols.b(end) - 1) / sqrt(vcov(end,end));
results.crit  = ztcrit(results.n,p);
results.meth   = 'adf';
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
...
%
% Output: 
%  crit    : Col vector of critical values [1% 5% 10% 90% 95% 99%] quintiles
